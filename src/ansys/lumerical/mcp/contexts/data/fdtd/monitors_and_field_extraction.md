...
1. `addmodeexpansion` over the same plane; link it to the monitor
   via the `"monitors for expansion"` property.
1. Set the modal cross-section (`"mode selection"`) the same way
   a mode source picks its mode.
1. After `run()`, read `getresult("<mode_expansion_name>", "expansion for ...")` and index `"a"` (forward) and `"b"`
   (backward) amplitudes.
