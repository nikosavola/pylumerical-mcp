...
1. **Add** structures, sources, and monitors.
1. **Save** the project file (`<handle>.save(<path>)`).
1. **Run** the simulation -- only after the user has explicitly
   confirmed (see "Running Simulations" below).
1. **Extract** results via lumapi getters (see the product
...
1. **Use the MCP tools for session lifecycle.** Use
   ...
   registry.
1. **Datasets, discovery, and re-running** are product-specific:
