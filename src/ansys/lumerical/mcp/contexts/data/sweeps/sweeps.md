...
1. `addsweep(<type>)` -- create the task.
1. `setsweep("<name>", "<property>", <value>)` -- configure.
1. `addsweepparameter("<name>", <param>)` -- add swept
...
1. `addsweepresult("<name>", <result>)` -- add result datasets.
1. `runsweep("<name>")` -- launch. Blocks until every child
   simulation completes.
1. `getsweepresult("<name>", "<result>")` -- pull results.
...
a new one with an auto-incremented name (`"sweep"`, `"sweep 1"`, ...). Re-running a setup snippet then silently accumulates
...
To remove an added parameter, use `removesweepparameter("<task>", "<parameter_name>")` (or the row index for S-parameter sweeps).
To remove an added result, use `removesweepresult("<task>", "<result_name>")`.
