...
1. **Discover before configure** -- call `ic.library()` and verify
...
1. **Add elements and position them** -- `addelement` + immediate
...
1. **Discover ports and connection roles** -- use `getports()` for
   every element to build the connection plan and identify any
   digital-to-electrical bridges needed.
1. **Connect elements** -- wire compatible ports with `connect()`.
1. **Configure elements and root mode** -- set element properties only
...
1. **Save** -- `ic.save(path)`.
1. **Run** -- `ic.run()` (only after user confirmation).
1. **Extract results** -- discover dataset names first, then extract
...
1. The property is currently active for the element's current mode.
1. If it is an enum, the value is one of the allowed choices.
1. If it is numeric, the supplied units/range match the property table.
